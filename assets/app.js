/*
  ...
  Zoom/pan control (Phase 3+):
  - scaleLimit enforces min/max zoom via ECharts native
  - wheel + pinch use chart.dispatchAction({ type:'geoRoam' }) so geo + all
    dependent series update in ONE synchronous render pass — eliminates the
    split-frame artifact where nodes move before the map
  - wheel events are RAF-throttled: accumulate factors within a frame, apply once
  - georoam event clamps pan: at global zoom no panning; at higher zooms
    geographic bounds prevent the map escaping the viewport entirely
  - trailLength:0 on lines effect — removes particle streak artifacts during roam
*/
...
// ── Zoom/pan constants ────────────────────────────────────────────────────────
const GEO_MIN_ZOOM = 1.2;
const GEO_MAX_ZOOM = 20;
// Each wheel tick zooms by this multiplier. 1.08 = 8% per tick (slow & precise).
const WHEEL_STEP = 1.08;
// Pinch: apply only this fraction of the raw gesture ratio (0.35 = 35% speed).
const PINCH_DAMPING = 0.35;
// Default global center (longitude, latitude)
...
function arrKey(arr) { return [...(arr || [])].slice().sort().join(','); }
...
function normalizeData(raw) {
  ...
  const normNodes = nodes.map((n) => {
    ...
    return {
      id, name: n.name, type: n.type || inferType(n.name),
      geoName: n.geoName ?? null, theaters: n.theaters || [], coords: n.coords,
      category: n.category, ihl: n.ihl || null, exposure: n.exposure || null,
...
function ensureEvidenceContainer() {
  ...
  c.className = 'mb-4';
  el('dual-classification-badges').insertAdjacentElement('afterend', c);
  return c;
}
...
function normalizeSourceItem(src) {
  ...
  if (typeof src === 'string') {
    ...
    return { title: isUrl ? s.replace(/^https?:\/\//i, '') : s, url: isUrl ? s : null, publisher: null, published_at: null, accessed_at: null, note: null, citation: !isUrl ? s : null };
  }
  if (typeof src === 'object') {
    ...
    return {
      title, url, publisher: typeof src.publisher === 'string' ? src.publisher : null,
...
function renderEvidence(container, { updated_at, confidence, sources } = {}) {
  ...
  const lastVal = document.createElement('div'); lastVal.className = 'mt-0.5'; lastVal.textContent = last;
  ...
  const confVal = document.createElement('div'); confVal.className = 'mt-0.5'; confVal.textContent = String(conf);
  ...
  if (!srcNorm.length) {
    ...
    for (const s of srcNorm.slice(0, 10)) {
      ...
      if (s.note || s.citation) {
        const note = document.createElement('div'); note.className = 'mt-1 text-[11px] text-slate-300'; note.textContent = s.note || s.citation; li.append(note);
      }
      srcList.append(li);
    }
  }
...
function defaultAbout(meta, datasetMeta) {
  ...
  return {
    title, subtitle: 'An editorial map of active conflicts and conflict-connected power moves.', last_reviewed_at: last,
    sections: [
    ...
    caveats: {
      ...
      bullets: ["This is an editorial project. Categories and summaries reflect the author's assessment at the time of writing.",'IHL labels are included as a reference framework, not as legal determinations or official rulings.','Terms like "war", "self-defense", or "genocide" can be contested and may change as evidence develops and formal processes conclude.','Use sources and timestamps: every node/edge can carry confidence + citations (expand "Evidence" in the panel).']
    }
  };
}
...
function setupAboutModal({ meta, datasetMeta }) {
  ...
  if (caveats) {
    ...
    const sum = document.createElement('summary'); sum.className = 'cursor-pointer list-none select-none text-sm font-semibold text-slate-200'; sum.textContent = caveats.heading || 'Caveats & Definitions (click to expand)'; det.append(sum);
...
function initUI({ meta, data, events }) {
  ...
  el('ui-events-date').textContent = eventsFreshness ? `Events: ${formatDate(eventsFreshness)}` : 'Events: not refreshed yet';
  ...
  const lensHint = document.createElement('div'); lensHint.id = 'lens-hint'; lensHint.className = 'text-[10px] text-slate-400'; lensHint.textContent = '';
  ...
  const renderLinePanel = (lineDatum) => {
    ...
    badge.style.background = `${lineDatum.color}30`; badge.style.color = lineDatum.color; badge.style.border = `1px solid ${lineDatum.color}`;
    badge.textContent = lineDatum.type; badges.append(badge);
  ...
  const renderNodePanel = (nodeId) => {
    ...
    policy.style.background = `${node.color}30`; policy.style.color = node.color; policy.style.border = `1px solid ${node.color}`;
    policy.textContent = `Policy: ${node.exposure ?? '—'}`; badges.append(policy);
    ...
    const list = el('info-connections'); list.replaceChildren();
    if (edges.length) {
      ...
      for (const e of edges) {
        ...
        const dir = document.createElement('span'); dir.className = 'text-[10px] uppercase font-bold text-slate-400 bg-slate-700/50 px-1 rounded'; dir.textContent = (e.from === nodeId) ? '→ To' : '← From';
    ...
    } else { connectionsContainer.classList.add('hidden'); }
    ...
    if (related.length) {
      ...
      for (const evt of related) {
        ...
        if (safeEventUrl) { a.href = safeEventUrl; a.target = '_blank'; a.rel = 'noreferrer'; }
        a.className = 'font-semibold text-slate-200 hover:underline'; a.textContent = evt.headline || 'Untitled event';
        const metaRow = document.createElement('div'); metaRow.className = 'text-xs text-slate-400 mt-1';
        metaRow.textContent = evt.published_at ? formatDate(evt.published_at, { includeTime: true }) : '';
        li.append(a, metaRow); evList.append(li);
      }
  ...
  const updateMap = () => {
    ...
    const mapNodes = visibleNodes.map((n) => {
      ...
      return { name: n.name, value: n.coords.concat([size]), itemStyle: { color: n.color, opacity, borderColor: isSelected ? '#ffffff' : n.color, borderWidth: isSelected ? 2 : 1, shadowBlur: isConnected ? 10 : 0, shadowColor: n.color }, nodeId: n.id, ihl: n.ihl, status: n.exposure, details: n.summary };
    });
    ...
    const mapLines = filteredEdges.map((e, idx) => {
      ...
      return { coords: [fromNode.coords, toNode.coords], fromName: fromNode.name, toName: toNode.name, type: e.type, summary: e.summary, color: e.color, edgeId: e.id, updated_at: e.updated_at, confidence: e.confidence, sources: e.sources, lineStyle: { color: e.color, width: lineWidth, type: e.effect, curveness, opacity } };
    }).filter(Boolean);
    const mapFills = visibleNodes.filter((n) => (n.type === 'state' || n.type === 'territory') && n.geoName).map((n) => ({ name: n.geoName, itemStyle: { areaColor: n.color + (state.selected && !connectedNodes.has(n.id) ? '05' : '30'), borderColor: n.color + (state.selected && !connectedNodes.has(n.id) ? '10' : 'ff') } }));
    ...
    chart.setOption({
      ...
      series: [
        ...
        { id: 'ambientNodes', name: 'ambientNodes', type: 'scatter', coordinateSystem: 'geo', geoIndex: 0, zlevel: 2, symbolSize: (val) => val[2], data: ambientNodes },
        {
          ...
          // trailLength:0 prevents particle streaks during zoom/pan
  ...
  if (state.selected && data.nodesById.get(state.selected)) { initialCenter = data.nodesById.get(state.selected).coords; initialZoom = 4; }

  chart.setOption({
    backgroundColor: 'transparent',
    geo: {
      id: 'baseGeo', map: 'world', roam: true,
      zoom: initialZoom, center: initialCenter,
      // Hard zoom floor and ceiling — ECharts enforces these on native roam too
  ...
  // ── Zoom & pan control ────────────────────────────────────────────────────
  //
  // ZOOM: We intercept wheel/pinch in capture phase to prevent ECharts' own
  // handler from firing, then apply the dampened factor via dispatchAction.
  // dispatchAction({ type:'geoRoam' }) updates geo + all dependent series in
  // ONE synchronous render pass — this eliminates the split-frame artifact
  // where nodes jump ahead of the map tile.
  //
  // PAN: The georoam event fires after every roam (native drag or our dispatch).
  // We clamp the center: at global zoom no panning; at higher zooms we enforce
  // geographic bounds so the map can't fully leave the viewport.

  const mapEl = el('map-container');

  const getGeoZoom = () => chart.getOption()?.geo?.[0]?.zoom ?? GEO_MIN_ZOOM;

  // Apply zoom via dispatchAction — keeps geo + series in sync
  const handleZoom = (factor, pixelX, pixelY) => {
    const curZoom = getGeoZoom();
    const targetZoom = Math.max(GEO_MIN_ZOOM, Math.min(GEO_MAX_ZOOM, curZoom * factor));
    const clampedFactor = targetZoom / curZoom;
    if (Math.abs(clampedFactor - 1) < 0.0001) return;
  ...
  // RAF throttle: accumulate wheel events within one frame, apply once.
  // Prevents the browser firing 8-12 wheel events per physical tick on
  // a Mac trackpad, which previously caused rapid-fire setOption calls.
  let rafId = null;
  let pendingFactor = 1;
  let pendingOriginX = 0;
  let pendingOriginY = 0;

  mapEl.addEventListener('wheel', (e) => {
    e.preventDefault();
    e.stopImmediatePropagation(); // block ECharts' bubble-phase handler

    const rect = mapEl.getBoundingClientRect();
    const step = e.deltaY < 0 ? WHEEL_STEP : (1 / WHEEL_STEP);

    // Accumulate factors — multiplying is correct for zoom composition
    pendingFactor *= step;
    pendingOriginX = e.clientX - rect.left;
    pendingOriginY = e.clientY - rect.top;

    if (!rafId) {
      rafId = requestAnimationFrame(() => {
        handleZoom(pendingFactor, pendingOriginX, pendingOriginY);
        pendingFactor = 1; // reset accumulator
        rafId = null;
      });
    }
  }, { passive: false, capture: true });

  // Pinch (iPhone / two-finger trackpad)
  ...
  // Pan bounds: fires after every roam (native drag OR our dispatchAction).
  // Guards against recursion with panClampActive flag.
  let panClampActive = false;

  chart.on('georoam', () => {
    if (panClampActive) return;

    const opt = chart.getOption();
    const geo = opt.geo?.[0];
    if (!geo?.center) return;

    const zoom = geo.zoom ?? GEO_MIN_ZOOM;
    let [lng, lat] = geo.center;

    // ── At or near global zoom: no panning ──────────────────────────────
    // The threshold (min × 1.15 ≈ 1.38) gives a small grace zone so that
    // zooming out from 1.5 back to minimum doesn't snap the center until
    // the user is truly at the overview level.
    if (zoom <= GEO_MIN_ZOOM * 1.15) {
      ...
      if (dLat > 3 || dLng > 8) {
        ...
        panClampActive = false;
      }
      return;
    }

    // ── At higher zooms: clamp to reasonable geographic bounds ──────────
    // These prevent the map from disappearing off-screen entirely when
    // panning at any zoom level, while allowing free exploration.
    const clampedLat = Math.max(-75, Math.min(82, lat));
    const clampedLng = Math.max(-200, Math.min(250, lng));
